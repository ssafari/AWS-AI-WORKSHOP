...
# it to execute the function with the specified permissions.
def add_aws_iam_role_and_policy(service_iam_role, 
                                service_policy_role, 
                                iam_role_name, 
                                policy_role_name):
    ''' Create IAM Role and Policy for a service '''
    try:
        iam = boto3.client("iam")
        # Create IAM Role and Policy
        role_name = iam.create_role(
            RoleName=iam_role_name,
            AssumeRolePolicyDocument=json.dumps(service_iam_role)
        )['Role']['RoleName']

        policy_arn = iam.create_policy(
            PolicyName=policy_role_name,
            PolicyDocument=json.dumps(service_policy_role)
        )['Policy']['Arn']
